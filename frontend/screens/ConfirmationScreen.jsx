...
const ConfirmationScreen = () => {
    ...
    const handlePlaceOrder = async () => {
        try {
            const response = await axios.post(
                ...
                { userId: userIdFromToken, cartItem: cart, shippingAddress: selectedAddress, paymentMethod, totalPrice: total }
            )
    ...
    async function pay() {
        try {
            const response = await axios.post('http://192.168.43.207:8000/api/v1/payments/intents',
                { amount: total * 100 },
                {
                    headers: {
            ...
            // console.log("response.data : ", response.data.paymentIntent);
            
            const { client_secret } = response.data.paymentIntent;
            console.log("clientSecret ", client_secret);
            setClientSecret(client_secret);
            setPaymentMethod(response.data.paymentIntent.payment_method_types[0])
    ...
    const initializePaymentSheet = async () => {
        const { error } = await initPaymentSheet({
            ...
            merchantDisplayName: 'Ankur swami',
            allowsPaymentMethodsRequiringShipping: true,
        });
        if (!error) {
            openPaymentSheet();
        }
    ...
    return (
        <StripeProvider publishableKey="pk_test_51PKx14SIhQBv3dCORFnXIQHMIVeYjODMJOiXfJPj9G0JkBqG9qwWHxPzLok9otYPa1r99w0QqWUnIjCuy5RNOdw900qPDE8V7z">
